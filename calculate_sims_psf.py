#/usr/bin/env python3
# run sextractor and psfex to extract psf info from these files.  Config files
# are in the ../Niall dir
import sys
import subprocess
import os
...
sims_dir = 'sims'
weights_dir = "/Users/adamwheeler/Dropbox/y1_test"
nchips = 30
...
for index in [str(i+1).zfill(2) for i in range(nchips)]:
    ...
    sex_cmd = ("sex "\
            ...
            + "-WEIGHT_IMAGE {wdir}/DECam_00241238_{i}.fits'[2]' "\
            + "-CATALOG_NAME {sims}/{prefix}_{i}_psfcat.fits "\
            + "-PARAMETERS_NAME {config}/psfex.param "\
            + "-FILTER_NAME {config}/default.conv"\
            ).format(config=config_dir, i=index, sims=sims_dir, wdir=weights_dir, prefix=prefix)
    print(sex_cmd)
